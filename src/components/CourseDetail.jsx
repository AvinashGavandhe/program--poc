...
const CourseDetail = () => {
  ...
  const dummyReplyContent = {
    data: {
      type: "reply",
      attributes: {
        dateCreated: "2023-03-13T11:36:34.000Z",
        dateUpdated: "2023-03-13T11:36:34.000Z",
        downVote: 0,
        richText: "<p>reply to test avinash</p>",
        state: "ACTIVE",
        text: "reply abuse",
        upVote: 0,
      },
  ...
  const handleReply = async (commentId) => {
    ...
    const apiUrl = `https://learningmanagereu.adobe.com/primeapi/v2/comments/23132/replies`;
    const response = await axios({
      data: dummyReplyContent,
      method: "POST",
      url: apiUrl,
    });
    const status = response;
  ...
  const getReplyArray = async (commentId) => {
    console.log("commentReply", commentId);
    // const apiUrl = `https://learningmanagereu.adobe.com/primeapi/v2/comments/${commentId}/replies?page[offset]=0&page[limit]=10&sort=-dateUpdated`;
    // const response = await axios.get(apiUrl);
    // console.log("comment", response?.data?.data);
    // setReplyArray(response?.data?.data);
  };
  ...
  const handleDisLike = async (commentId) => {
    //https://learningmanagereu.adobe.com/primeapi/v2/comments/23065/vote?action=DOWN
              ...
              {commentsArray?.map((comment) => {
                ...
                return (
                  <>
                    ...
                    {/*start show reply section  */}

                    <button onClick={() => getReplyArray(comment?.id)}>
                      Show Replies
                    </button>

                    {/*end show reply section  */}
